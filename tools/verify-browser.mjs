...
try {
  ...
  await expect(cdp, "desktop payment methods in one row", "getComputedStyle(document.querySelector('.payment-grid')).gridTemplateColumns.split(' ').length === 4 && [...document.querySelectorAll('.payment-card')].every((card) => { const image = card.querySelector('img').getBoundingClientRect(); const ratio = image.width / card.getBoundingClientRect().width; return ratio >= 0.99 && ratio <= 1.01 && image.height / image.width > 0.74 && image.height / image.width < 0.76; })");
