...
try {
  ...
  await expect(cdp, "desktop payment cards stacked full screen width", "getComputedStyle(document.querySelector('.payment-grid')).gridTemplateColumns.split(' ').length === 1 && [...document.querySelectorAll('.payment-card')].every((card) => { const cardBox = card.getBoundingClientRect(); const image = card.querySelector('img').getBoundingClientRect(); const gridWidth = document.querySelector('.payment-grid').getBoundingClientRect().width; return cardBox.width >= gridWidth - 2 && image.width / cardBox.width >= 0.99 && image.height / image.width > 0.74; })");
